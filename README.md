...
- 🔌 **MCP适配**: 集成Model Context Protocol，支持工具调用------目前仅内置 filesystem fetch 两个内置MCP,后续可升级为可配置MCP
- 📊 **会话记忆**: 窗口级别短期记忆，支持Summary Memory”-----当前触发条件为2000 token 3条消息，keep-保持20条消息压缩
- ⚡ **流式返回**: 实时流式响应，提升用户体验
- ⚡ **取消请求**: 支持用户在流式响应过程中取消请求
